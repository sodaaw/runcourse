...
import { MapPin, Star, Droplets, ShoppingBag, ToiletIcon, Moon } from "lucide-react";
...
export default async function CourseDetailPage({
  ...
  return (
    <div className="mx-auto max-w-content px-6 py-8 pb-44 sm:pb-28">
      ...
      <h1 className="mt-1 text-[26px] font-semibold leading-tight text-ink sm:text-3xl">
        {course.name}
      </h1>
      ...
      <div className="mt-4 flex flex-wrap items-center gap-4 tabular-nums">
        ...
        {course.timeOfDay.includes("night") && (
          <span className="flex items-center gap-1 rounded-full bg-night px-2.5 py-1 text-xs font-medium text-white">
      ...
      <section className="mt-8">
        <h2 className="text-base font-semibold text-ink">코스 지도</h2>
        ...
        <div className="mt-4">
          <KakaoMap
            ...
            className="h-72 w-full rounded-xl border border-line sm:h-96"
          />
        </div>
      </section>
      ...
      {/* Elevation */}
      <section className="mt-8">
        <h2 className="text-base font-semibold text-ink">고도 프로필</h2>
      ...
      <section className="mt-10">
        <h2 className="text-base font-semibold text-ink">편의시설</h2>
        <ul className="mt-3 flex flex-col gap-2">
          {course.amenities.map((a, i) => {
            ...
            return (
              <li
                key={i}
                className="flex items-center gap-2.5 rounded-xl border border-line px-4 py-3 text-sm text-ink"
              >
